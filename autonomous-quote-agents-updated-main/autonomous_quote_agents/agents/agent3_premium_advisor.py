...
def analyze(record: dict, risk_tier: str, bind_probability: float) -> dict:
    ...
    # Affordability threshold: >2.5% of annual income → likely blocker (increased from 2.0%)
    if pct_of_income > 2.5:
        is_blocker = True
        reason_log.append(f"Premium ({quoted_premium:.0f}) = {pct_of_income:.1f}% of annual income — exceeds 2.5% affordability threshold")
